---
name: anti-slop
description: Use when writing or reviewing any prose in this repo - commit messages, PR and issue text, specs, plans, docs, code comments, and user-facing strings - and when shaping chat replies. Triggers include "write the commit", "draft the PR", "write the spec", "review the wording".
---
...
sentences, active voice, simple words, one fact per sentence, and the same term for the
same thing. Write "must" for a requirement, "can" for a possibility, and no contractions.

The word limits apply to a sentence a change writes, not to one it edits in place: 25 words
for a descriptive sentence, 20 for a procedural step, 6 sentences for a paragraph, 3 words
for a noun cluster ("restart backoff delay", not "server crash restart backoff delay").
Specs and plans under `docs/superpowers/` follow the base style without these limits.
...
- **Why:** abstract layering hides whether there is a real claim underneath.
...
## 16. No AI vocabulary
- **Don't:** "delve", "crucial", "pivotal", "leverage", "harness", "robust", "seamless", "comprehensive", "cutting-edge", "streamline", "journey", "holistic", "myriad", "plethora", "state-of-the-art", "meticulous", "profound", "transformative", "unprecedented", "intricate", "multifaceted", "nuanced", "vital", "elevate", "empower", "foster", "facilitate", "bolster", "garner", "glean", "spearhead".
...
- **Do:** the specific word. Not "robust error handling" but "retries five times, then fails the run". Most entries have a plain replacement: "vital" is "required", "facilitate" is "run", "glean" is "read", "elevate" is "raise", "bolster" is "add to". "Myriad" and "plethora" are the count. "Foster", "empower", and "transformative" name no action: write what the change does. The exemptions of rule 25 apply here too.
...
## 34. UI text states the effect
- **Don't:** in user-visible strings: "Opens a picker next to the label." / "Drag to adjust." / "The choice is kept across restarts."
- **Why:** all three describe something the user already has: the control on the screen, its obvious handling, a property every setting shares.
- **Do:** say what the control does, nothing else: "Sets how many sessions the server accepts at once." An instruction stays only when it reveals something invisible, like when a change takes effect: "Changes apply the next time the server starts." Help text whose subject is the layout itself states where a control sits; that location is its content. User-visible strings only; developer docs name controls by the label they carry in the UI and may describe widgets.

## 35. No empty qualifiers
- **Don't:** "Returns the appropriate handler for the given input.", "the relevant file", "any necessary cleanup", "as needed", "if applicable", "where appropriate", "properly configured".
- **Why:** the adjective narrows nothing. Delete it and the sentence means the same, which is the test for this class.
- **Do:** name the constraint: "Returns the handler registered for the file extension." When no constraint exists, drop the adjective and keep the noun. The exemptions of rule 25 apply here too.

## 36. No non-reasons
- **Don't:** "for performance reasons", "for legacy reasons", "for safety reasons", "due to the nature of the API", "this is by design", "as expected".
- **Why:** the phrase replaces the reason and implies that a reason is known.
- **Do:** the named reason: "The stop target checks pgrep first because an unguarded AppleScript quit would launch the app." When the reason is lost, write "nobody remembers why"; that tells the next reader what to expect. The exemptions of rule 25 apply here too.

## 37. No self-praise in code comments
- **Don't:** "this ensures correctness", "handles this gracefully", "for readability", "follows best practices", "the cleanest way to do this".
- **Why:** the comment claims a quality that the reader cannot check and the compiler does not enforce.
- **Do:** state what breaks without the line: "Without the explicit sort, the order follows the directory read order and changes between launches." Rule 33 is this failure in a chat reply. The exemptions of rule 25 apply here too.

## 38. No preface frames
- **Don't:** "Note that", "Note:", "Please note", "under the hood", "behind the scenes", "at a high level", "in essence", "in other words", "keep in mind", "as mentioned above"; and the stock connectives "Moreover,", "Furthermore,", "Additionally,", "Notably,", "Consequently,", "That said,".
- **Why:** the frame announces a sentence instead of being one, and the connective names a relation the two sentences already state.
- **Do:** read the sentence without the frame. Keep the frame only when the sentence loses a fact, which is rare. Rule 4 bans announcement verbs and rule 10 bans closing formulas; this rule covers the opener in front of, or between, sentences. The exemptions of rule 25 apply here too.

## 39. No templated headings
- **Don't:** "Why X Matters", "Understanding X", "A Deep Dive into X", "Demystifying X", "Best Practices", "Key Takeaways", "5 Things You Need to Know".
- **Why:** the heading names a shape and leaves the reader to guess what the section holds.
- **Do:** the heading states the subject or the finding: "Quit stops the servers" instead of "Understanding the server lifecycle". A heading that states the finding lets a reader skim the document and get the argument. The exemptions of rule 25 apply here too.
